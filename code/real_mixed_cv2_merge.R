...
files <- list.files('real_mixed_cv2', full.names = T)
print('The second-stage bandwidths for inpatient, outpatient, skilled nursing, home health, hospice and total costs are:')
for (type in c('ip', 'op', 'sn', 'hh', 'hs', 'all')) {
    result <- c()
    for (file in files) {
        if (startsWith(file, paste0('real_mixed_cv2/', type))) {
            temp <- readRDS(file)
