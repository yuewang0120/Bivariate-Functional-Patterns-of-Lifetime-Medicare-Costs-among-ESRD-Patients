...
fit <- with(data, kfit.p(matrix(1, sum(!test)), pres[!test], (tau - tspl)[!test], (end - tau)[!test], (tau - tspl)[test], (end - tau)[test], h, 19))
res <- with(data[test, ], y - c(fit))
dir.create('simu_mixed_cv2')
data.frame(h = h, fold = fold, sse = sum(res^2, na.rm = T), nobs = sum(!is.na(res))) %>% saveRDS(paste0('simu_mixed_cv2/seed=', seed, '_h=', h, '_fold=', fold))
